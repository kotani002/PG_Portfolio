...
- **更新日表示**（ヘッダー右上）
- **スマホ表示対応**（ハンバーガーメニュー）
- **ダークモード切り替え**（ヘッダーのボタンでワンタップ切替。初回はOSの設定に自動追従し、以降は選択内容をブラウザに保存）
...
- ダークモードの配色は `style.css` の `[data-theme="dark"]` ブロックで定義しています。色を調整したい場合はここの値を変更してください（他の色は `:root` 側の変数を編集すればライトモードに反映されます）。
